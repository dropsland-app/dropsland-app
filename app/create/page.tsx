...
import { useWallets } from "@privy-io/react-auth"; // Added for reliable address fetching
...
export default function CreatePage() {
  ...
  const { wallets } = useWallets(); // Direct access to wallets
  // You can still keep useAuth if you need other user metadata like username
  ...
  const handleUpload = async () => {
    ...
    // Get the connected wallet address via Privy
    const primaryWallet =
    ...
    try {
      ...
      // This ensures we have a record in the 'profiles' table before linking a track
      const { error: profileError } = await supabase.from("profiles").upsert(
        {
          wallet_address: walletAddress,
          username: artistName, // Use the form input as default or userData.username
          role: "DJ",
        },
      ...
      setTrackName("");
      // Keep artist name populated for convenience? Or reset:
      // setArtistName("");
      // setGenre("");
    } catch (error: any) {
